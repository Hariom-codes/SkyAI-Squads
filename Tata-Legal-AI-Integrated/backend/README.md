...
## Configuration notes
- Set `GEMINI_API_KEY` in the backend `.env`.
- Set `GEMINI_MODEL=gemini-3.5-flash-lite` (or another currently available Gemini model).
- If Poppler is not on PATH, set `POPPLER_PATH` to your local Poppler `Library\bin` directory.
- The frontend upload/review flow now consumes the `/upload` JSON response instead of `src/data/mockData.js`.

## Performance optimization
The analysis pipeline is optimized so the 30-PDF knowledge base is **not rebuilt on every upload or startup**. The existing ChromaDB is opened directly. Digital PDFs use PyMuPDF text extraction first; OCR is used only for pages that appear scanned. Knowledge-base embeddings are batched, and clause analysis is grouped into parallel Gemini requests. This avoids the old one-Gemini-call-per-clause pattern.
